...
///
/// 2. **Why `push_added_item` upgrades a tombstone in place.** An
///    Inbox→Inbox yield emits `RemovedFromInbox` immediately followed by
///    `EnqueuedToInbox` for the same key (when `set_run_at(None)` leaves
///    the key unchanged). The storage commits land in the same order
///    *before* the notifications dispatch, so the row is present in the
///    final state. The overlay therefore replaces the tombstone with an
///    `Add(value)`. A second add without an intervening tombstone is
///    still treated as an invariant violation and panics.
///
...
struct RefillTask {
    ...
    /// Cooperative cancellation flag shared with the spawned blocking task.
    /// `tokio::task::spawn_blocking` tasks are not abortable, so dropping the
    /// `JoinHandle` only detaches the work. We instead set this flag in
    /// `Drop` and have the worker check it between rows so it exits promptly
    /// and frees its blocking-pool slot.
...
impl RefillTask {
    ...
    fn push_added_item(&mut self, key_to_add: &EntryKey, value: &EntryValue) {
        let pos = match self.overlay.binary_search_by_key(key_to_add, |&(k, _)| k) {
            Ok(pos) => {
                let entry = self.overlay.get_mut(pos).unwrap();
                match entry.1 {
                    // Tombstone → Add: an Inbox→Inbox yield emits
                    // `RemovedFromInbox(k)` immediately followed by
                    // `EnqueuedToInbox { key: k, .. }` whenever
                    // `set_run_at(None)` leaves the key unchanged
                    // (see `yield_entry` in `lib.rs`). The final storage
                    // state has the row, so the overlay must reflect that.
                    Overlay::Tombstone => {
                        entry.1 = Overlay::Add(value.clone());
                    }
                    // Two adds for the same key without an intervening
                    // tombstone is a real invariant violation: the partition
                    // processor never re-enqueues a still-present key.
                    Overlay::Add(_) => panic!(
                        "duplicate enqueue for the same key without an intervening removal: {key_to_add:?}"
                    ),
                }
                return;
            }
    ...
    fn update_anchor(&mut self, new_anchor: Option<EntryKey>) {
        match self {
            ...
            RefillState::InFlight(_) => {
                ...
                // Dropping the `RefillTask` flips its cancel flag (see
                // `Drop for RefillTask`); the worker observes it between
                // rows and exits promptly. The `JoinHandle` itself only
                // detaches on drop — `spawn_blocking` tasks are not
                // abortable. See `RefillTask` doc for why discarding the
                // overlay is safe here.
...
impl<S: VQueueStore> Queue<S> {
    ...
    pub fn remove(&mut self, key_to_remove: &EntryKey) -> bool {
        ...
        let Ok(pos) = self.items.binary_search_by_key(key_to_remove, |&(k, _)| k) else {
            // Not in cache. The key is either:
            // - already shipped (in the unconfirmed-assignments set) or never
            //   in the inbox to start with — ignore;
            // - within the in-flight refill's coverage zone (`<= refill_anchor`)
            //   — also ignore, the refill's snapshot will reflect the deletion
            //   directly;
            // - above the in-flight refill's coverage zone — record as a
            //   tombstone in the overlay so the merge step can suppress it.
    ...
    pub fn enqueue(&mut self, key: &EntryKey, value: &EntryValue) -> bool {
        ...
        // `false` otherwise: the item is a duplicate, was inserted at a
        // non-front position, or was rejected because its key falls strictly
        // above the cache's coverage zone (`> refill_anchor`). A rejected item
    ...
    /// the next cached item from the in-memory cache. Refilling the cache from
    /// storage is the responsibility of `poll_advance_if_needed` /
    /// `try_refill`, not this method.
    ///
        ...
        loop {
            match stage {
                Stage::New {
                    ...
                    panic!(
                        "vqueue has no running items but its metadata says it has {}",
                        already_running.get(),
                    );
                }
                Stage::Running {
    ...
    ///
    /// Starting at the standby `refill_anchor` (or the very first key if no anchor
    /// is set yet). Returns true if items were added. Items in the `skip` set are
    /// not added to the cache, but the anchor advances past them so they are not
    /// reconsidered on the next refill.
    ///
    /// # Panics
    ///
    /// Panics if `refill_state` is not [`RefillState::Standby`]. Callers must
    /// ensure no refill task is already in flight.
    fn try_refill(
        &mut self,
        storage: &S,
        qid: &VQueueId,
    ...
    fn start_refill_task(&mut self, storage: &S, qid: &VQueueId) {
        assert!(!self.refill_state.is_in_flight());
        ...
        let handle = tokio::task::spawn_blocking(move || {
            ...
            while results.len() < INBOX_CACHE_CAPACITY {
                if task_cancel.load(Ordering::Relaxed) {
                    // Caller dropped the `RefillTask`; bail early so we
                    // don't keep the blocking-pool slot scanning rows
                    // nobody will read.
                    return Ok(results);
                }
                ...
                match reader.peek() {
                    ...
                    Err(CursorError::WouldBlock) => {
                        unreachable!("background refill task should never see WouldBlock");
                    }
                    Err(CursorError::Other(e)) => {
                        tracing::error!("refill task failed: {e}");
                        return Err(e);
                    }
                }
            }
    ...
    fn poll_refill_task(
        ...
        //
        //
        // The strategy here is more complex than the blocking/inline version because
            ...
            .merge_join_by(overlay, |(key, _), (overlay_key, _)| key.cmp(overlay_key))
        {
            ...
            match either {
                ...
                | EitherOrBoth::Both((key, _), (_, Overlay::Add(value))) => {
                    ...
                    // The merge stream is strictly above every key currently
                    // in `self.items`: storage rows come from
                    // `seek_after(refill_anchor)`, the overlay only stores
                    // keys `> refill_anchor` (see `push_added_item` /
                    // `push_tombstone`), and `self.items` only holds keys
                    // `<= refill_anchor`. Therefore every accepted key
                    // appends at the back.
                    debug_assert!(
                        self.items.back().is_none_or(|(k, _)| *k < key),
                        "merge output must sort strictly above existing cache items"
                    );
                    if self.items.len() >= INBOX_CACHE_CAPACITY {
                        // Cache is full; drop this row and let the next refill
                        // rediscover it via `seek_after(items.back())`.
